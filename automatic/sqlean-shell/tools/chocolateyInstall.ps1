...
if ((Get-ProcessorBits 32) -or $env:ChocolateyForceX86 -eq 'true') {
  Write-Error -Message "32-bit version of sqlean-shell not availabe" -Category ResourceUnavailable
}

$toolsDir   = Split-Path -parent $MyInvocation.MyCommand.Definition
$executable = Join-Path $toolsDir 'sqlean.exe'
...
if ($pp.count -gt 0) {
  ...
  if ($paths.Count -gt 0) {
    $icon = Join-Path $toolsDir 'sqlean.ico'

    $paths.GetEnumerator() | foreach-object {
      $shortcutPath = Join-Path $_ 'sqlean.lnk'
